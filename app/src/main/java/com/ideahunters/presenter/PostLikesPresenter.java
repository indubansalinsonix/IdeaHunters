...
public class PostLikesPresenter implements Constants {
    private final Context context;
    private final PostLikesPresenterListener mListener;
    private final ApiClient apiClient;
    private String device_type, status;
    private String image_name;
    private String file;

    public interface PostLikesPresenterListener {




        void onSuccessfulLike(String likecount, String message, String likebyme);

        void onUnSuccessfulLike(String message);

    }


    public PostLikesPresenter(PostLikesPresenterListener listener, Context context) {
    ...
    public void postLikes(String sug_id) {

     //   if (onValidate(message)) {
            apiClient
                    .createService(BASE_URL)
                    .postLikes(Singleton.getInstance().getValue(context, USER_ID),sug_id)
                    .enqueue(new Callback<Likes>() {
                        @Override
...
//                        Toast.makeText(context, response.body().getMessages().toString(), Toast.LENGTH_SHORT).show();
                            ...
                            if (response.body().getStatus().equals("true")) {
                                ...
                                mListener.onSuccessfulLike(response.body().getLikecount(),response.body().getMessage(),response.body().getLikebyme());
                          ...
                          /* sliderArrayList.clear();
                            ...
                            mListener.setPageImages(pageArrayList);*/
                                ...
                                mListener.onUnSuccessfulLike(response.body().getMessage().toString());
                            }
